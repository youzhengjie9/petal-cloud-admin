 ...
 */
@Service("sysQiniuOssUploadServiceImpl")
@Slf4j
public class SysQiniuOssUploadServiceImpl implements SysOssUploadService {

    private SysQiniuOssProperties sysQiniuOssProperties;

    @Autowired
    public void setSysQiniuOssProperties(SysQiniuOssProperties sysQiniuOssProperties) {
        this.sysQiniuOssProperties = sysQiniuOssProperties;
    }
    ...
    @Override
    public ResponseResult<Object> imageUpload(MultipartFile imageFile) {
        ...
        try {
            ...
            Auth auth = Auth.create(sysQiniuOssProperties.getAccessKey(), sysQiniuOssProperties.getSecretKey());
            String upToken = auth.uploadToken(sysQiniuOssProperties.getBucket());
            //七牛云oss上传文件的核心方法
            ...
            result.setData(sysQiniuOssProperties.getOssUrl()+newFileName);
            return result;
        }catch (Exception e){
            e.printStackTrace();
     ...
     */
    @Override
    public ResponseResult<Object> fileDelete(String fileFullName) {
        //封装响应结果
        ResponseResult<Object> result = new ResponseResult<>();
        ...
        try {
            //七牛云oss文件删除核心方法
            Response response = bucketManager.delete(sysQiniuOssProperties.getBucket(), fileFullName);
